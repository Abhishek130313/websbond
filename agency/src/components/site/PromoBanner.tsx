  ...
  <section className="container py-12 md:py-16">
    <div className="relative rounded-3xl bg-slate-950 border border-white/10 text-white overflow-hidden p-8 sm:p-10 lg:p-16 grid md:grid-cols-[1.1fr_0.9fr] items-center gap-10 shadow-2xl">
      ...
      <div className="absolute inset-0 opacity-30 pointer-events-none" style={{
        backgroundImage: "radial-gradient(circle at 80% 50%, hsl(38 97% 48% / 0.2) 0%, transparent 60%)",
      }} />
      <div className="absolute top-0 left-0 w-full h-full grid-mesh opacity-10 pointer-events-none" />

      <div className="relative z-10">
        <div className="inline-flex items-center gap-2 bg-amber-500/10 border border-amber-500/25 text-amber-300 font-semibold text-xs uppercase tracking-wider px-3 py-1.5 rounded-full mb-4">
        ...
        <div className="mt-8 flex flex-wrap gap-x-5 gap-y-2 text-xs text-muted-foreground">
          {["No Locked Contracts", "Direct Developer Access", "Cancel Anytime"].map((t) => (
            <div key={t} className="flex items-center gap-1.5"><Check className="w-4 h-4 text-amber-500" /> {t}</div>
          ))}
        </div>
      </div>
      ...
      <div className="relative z-10 bg-white/[0.02] border border-white/5 rounded-3xl p-6 md:p-8 backdrop-blur-sm shadow-inner flex flex-col gap-6">
        ...
        <div className="space-y-4">
          ...
          {/* Slot 3 */}
          <div className="flex items-center gap-4 bg-amber-500/10 border border-amber-500/20 rounded-2xl p-4 animate-pulse">
            <div className="w-8 h-8 rounded-full bg-amber-500/20 text-amber-500 grid place-items-center font-bold text-xs shrink-0 animate-bounce">
              3
            </div>
            <div className="flex-1">
              <div className="text-xs font-bold text-white flex items-center justify-between">
                <span>Slot 3: Available</span>
                <span className="text-[9px] bg-amber-500/20 text-amber-400 px-1.5 py-0.5 rounded uppercase font-semibold">Active</span>
              </div>
